...
public class revM22LogParser {
	...
	public static void main(String[] args) throws IOException{
		...
		FileWriter avgTTotalwriter = new FileWriter("M22/rev_ttotal_avg_"+num_servers+"_"+op_type+".csv"); 
		avgTTotalwriter.write("rep. factor,Avg. T Total (ms)\n");
		FileWriter avgTQueuewriter = new FileWriter("M22/rev_tqueue_avg_"+num_servers+"_"+op_type+".csv"); 
		avgTQueuewriter.write("rep. factor,Avg. T Queue (ms)\n");
		FileWriter avgTServerwriter = new FileWriter("M22/rev_tserver_avg_"+num_servers+"_"+op_type+".csv"); 
		avgTServerwriter.write("rep. factor,Avg. T Server (ms)\n");
		
		
		
		FileWriter stdTTotalwriter = new FileWriter("M22/rev_ttotal_std_"+num_servers+"_"+op_type+".csv"); 
		stdTTotalwriter.write("rep. factor,STDev T Total (ms)\n");
		FileWriter stdTQueuewriter = new FileWriter("M22/rev_tqueue_std_"+num_servers+"_"+op_type+".csv"); 
		stdTQueuewriter.write("rep. factor,STDev T Queue (ms)\n");
		FileWriter stdTServerwriter = new FileWriter("M22/rev_tserver_std_"+num_servers+"_"+op_type+".csv"); 
		stdTServerwriter.write("rep. factor,STDev T Server (ms)\n");
		
		
		FileWriter fiftyTTotalwriter = new FileWriter("M22/rev_ttotal_50p_"+num_servers+"_"+op_type+".csv"); 
		fiftyTTotalwriter.write("rep. factor,50 Percentile T Total (ms)\n");
		FileWriter fiftyTQueuewriter = new FileWriter("M22/rev_tqueue_50p_"+num_servers+"_"+op_type+".csv"); 
		fiftyTQueuewriter.write("rep. factor,50 Percentile T Queue (ms)\n");
		FileWriter fiftyTServerwriter = new FileWriter("M22/rev_tserver_50p_"+num_servers+"_"+op_type+".csv"); 
		fiftyTServerwriter.write("rep. factor,50 Percentile T Server (ms)\n");
		
		
		FileWriter ninetyTTotalwriter = new FileWriter("M22/rev_ttotal_90p_"+num_servers+"_"+op_type+".csv"); 
		ninetyTTotalwriter.write("rep. factor,90 Percentile T Total (ms)\n");
		FileWriter ninetyTQueuewriter = new FileWriter("M22/rev_tqueue_90p_"+num_servers+"_"+op_type+".csv"); 
		ninetyTQueuewriter.write("rep. factor,90 Percentile T Queue (ms)\n");
		FileWriter ninetyTServerwriter = new FileWriter("M22/rev_tserver_90p_"+num_servers+"_"+op_type+".csv"); 
		ninetyTServerwriter.write("rep. factor,90 Percentile T Server (ms)\n");
		
		
		FileWriter ninetynineTTotalwriter = new FileWriter("M22/rev_ttotal_99p_"+num_servers+"_"+op_type+".csv"); 
		ninetynineTTotalwriter.write("rep. factor,99 Percentile T Total (ms)\n");
		FileWriter ninetynineTQueuewriter = new FileWriter("M22/rev_tqueue_99p_"+num_servers+"_"+op_type+".csv"); 
		ninetynineTQueuewriter.write("rep. factor,99 Percentile T Queue (ms)\n");
		FileWriter ninetynineTServerwriter = new FileWriter("M22/rev_tserver_99p_"+num_servers+"_"+op_type+".csv"); 
			...
			for(int rep=1;rep<=5;rep++){
				...
				while(true){
					...
					if(split_s[0].equals(op_type)){
						repTTotal.add(Math.ceil(Long.parseLong(split_s[1])/1000000.0));
						repTQueue.add(Math.ceil(Long.parseLong(split_s[2])/1000000.0));
						repTServer.add(Math.ceil(Long.parseLong(split_s[3])/1000000.0));
					}
